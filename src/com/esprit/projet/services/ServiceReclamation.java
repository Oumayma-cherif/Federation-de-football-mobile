...
public class ServiceReclamation {
	...
	public ArrayList<Reclamation> getAllReclamation() {
        String url = BaseService.BASE_URL + "/mesReclamationsJson";
;
	...
	public boolean deleteReclamation(int id) {
        String url = BaseService.BASE_URL + "/supprimerReclamationJson/"+id;
        request.setUrl(url);
        request.setPost(false);
        request.addResponseListener(new ActionListener<NetworkEvent>() {
            @Override
	...
	public void ajouterReclamation(Reclamation p) {
       //try {
            ...
            cr.setUrl(BaseService.BASE_URL+"/ajouterReclamationJson");
            cr.setPost(false);
            cr.addArgument("objet", p.getObjet());
            cr.addArgument("descR", p.getdescR());
            cr.addArgument("user_id", ServiceUser.getSession().getId()+"");
	...
	private String modifyDateLayout(String inputDate) throws ParseException{
	 ...
	 public void updateReclamation(Reclamation p,int id) {
            ...
            MultipartRequest cr = new MultipartRequest();
            cr.setUrl(BaseService.BASE_URL+"/updateReclamationJson");
        cr.setPost(false);

            cr.addArgument("idr", id+"");
            cr.addArgument("objet", p.getObjet());
            cr.addArgument("descR", p.getdescR());
            cr.addArgument("status", p.getStatus());
            cr.addArgument("user_id", ServiceUser.getSession().getId()+"");
